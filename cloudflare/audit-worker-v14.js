      ...
      <div style="max-width:1100px;margin:0 auto">
        ...
        <div class="tz-hero" style="display:grid;grid-template-columns:auto 1fr auto;gap:24px;align-items:center">
          ...
          <div>
            ...
            ${(audit.detected_jurisdictions && audit.detected_jurisdictions.length) ? `<p style="margin:5px 0 0;font-size:0.66rem;color:rgba(248,245,239,0.6)">Assessed against the law of <strong style="color:rgba(248,245,239,0.85)">${esc(audit.detected_jurisdictions.slice(0,6).join(' · '))}</strong> — the markets your site shows you serve.</p>` : ''}
          </div>
...
function vizFrame(opts){
  ...
  return '<div style="background:#fff;border:1px solid '+VIZ.grid+';border-radius:8px;padding:14px 16px;display:flex;flex-direction:column">'
    +'<p style="margin:0 0 2px;font-family:\'Times New Roman\',serif;font-size:1rem;color:'+VIZ.ink+';font-weight:600">'+esc(opts.title)+'</p>'
    +'<p style="margin:0 0 6px;font-size:0.7rem;color:'+VIZ.muted+'">'+esc(opts.subtitle||'')+'</p>'
    +(opts.legend? vizLegend(opts.legend):'')
    +'<div style="margin-top:6px">'+opts.svg+'</div>'
    +(opts.howto? '<p style="margin:8px 0 0;font-size:0.64rem;color:'+VIZ.muted+';line-height:1.4"><strong style="color:'+VIZ.ink+'">How to read this:</strong> '+esc(opts.howto)+'</p>':'')
    +'</div>';
}
  ...
  return `
    <section class="tz-reveal" style="padding:16px 24px;background:#1F2937;color:rgba(248,245,239,0.6);font-size:0.7rem;line-height:1.55">
      <div style="max-width:1100px;margin:0 auto">
        ...
        <p style="margin:0">Each item flags a <em>potential</em> gap to review with a regulated solicitor or barrister before acting. Framework catalogue v${esc(String(audit.framework_version||'7.4'))}${audit.scan_meta && audit.scan_meta.generated_at ? ' · assessed '+esc(new Date(audit.scan_meta.generated_at).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})) : ''}. Produced by Tamazia Ltd, C1, Barking Wharf Square, London, IG11 7ZQ. Marketing diagnostic only.</p>
      </div>
    </section>`;
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    let html; const _v21 = url.searchParams.get('v')==='21' || (env && env.RENDER_V21==='1');
    try { html = _v21 ? renderV21Page(adapt(res.row)) : renderPage(adapt(res.row), url.href); }
    ...
    return new Response(html, { status: 200, headers: { 'content-type': 'text/html;charset=utf-8', 'cache-control': 'public,max-age=120', 'x-tamazia-audit': (_v21?'v21-preview':'v20-live-v15-p5') } });
  }
};

export { adapt, renderPage, renderEmailTeaser, loadAudit, computeRiskScore, assessDimensions, scoreFromDims };

// ===== v21 preview render (behind ?v=21 / env.RENDER_V21) — billionaire-grade Claude-aesthetic =====
function renderV21Page(audit){
  const merged = dedupeAndMerge(audit.pointers||[]);
  const dims = assessDimensions(merged, audit.signals||{}, audit.ai_readiness);
