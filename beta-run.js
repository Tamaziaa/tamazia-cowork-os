...
(async () => {
  ...
  const _out=_path.join(_fs.mkdtempSync(_path.join(_os.tmpdir(),'tamazia-')),'beta-results.json');
  _fs.writeFileSync(_out, JSON.stringify(results,null,2));
  console.log(`results: ${_out}`);
  process.exit(0);
