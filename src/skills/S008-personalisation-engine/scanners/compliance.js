...
function ruleCheck(rule, corpus, sector, corpusIndex) {
  ...
  // false-positive GDPR cascade on a French site whose /confidentialite page fell past the crawl cap).
  {
    ...
    if (_isDP) {
      // A page counts as a genuine privacy/data-protection policy ONLY if it is identified by CONTENT or by a
      // CLEAN policy URL path — never by a bare URL-substring. Two earlier false-positive cascades on ramsaysante.fr
      // came from substring matching: /test-cookie (the word "cookie") and /actualites/...-en-confidentialite (a NEWS
      // article whose slug contained "confidentialite"). Both are NOT privacy policies. We now require real evidence.
      // (1) CONTENT signal: a page body carrying >=2 distinct data-protection policy markers (multilingual) is a real
      //     policy page. A news article or cookie-test page will not carry two of these.
      const _DP_MARKERS = [
      ...
      // (2) CLEAN policy URL path: the policy term is a full path segment (not embedded in a long article slug) AND the
      //     page is not under a news/blog/press section. Guards against /actualites/...-confidentialite style slugs.
