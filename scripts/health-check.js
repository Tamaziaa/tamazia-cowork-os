...
function run() {
  ...
  // ---- MAILDECK COLD FLEET (§14.8) ----
  const mbLoaded = num(`SELECT COUNT(*) FROM mailbox_pool WHERE status='active'`);
  if (mbLoaded == null) rec('maildeck_pool_loaded', 'send', 'warn', 'mailbox_pool not initialised yet — run load-maildeck-creds.js after exporting the 30 MailDeck creds', 0);
  else if (mbLoaded === 0) rec('maildeck_pool_loaded', 'send', 'warn', 'no MailDeck cold mailboxes loaded yet (export creds → node scripts/load-maildeck-creds.js)', 0);
  else rec('maildeck_pool_loaded', 'send', 'ok', `${mbLoaded} cold mailboxes active in the pool`, mbLoaded);
  if (mbLoaded != null) {
    // CRITICAL compliance guard: a cold send must NEVER leave from a real brand domain.
    band('cold_real_domain_guard', 'send', num(`SELECT COUNT(*) FROM sends WHERE relay_used='maildeck' AND sent_at>NOW()-INTERVAL '7 days' AND LOWER(SPLIT_PART(COALESCE(mailbox_address,''),'@',2)) IN ('tamazia.co.uk','tamazia.in')`), 1, 1, v => v === 0 ? 'no cold sends from real brand domains (correct)' : `${v} cold sends LEFT FROM A REAL DOMAIN — investigate now`);
    // Ramp/cap integrity: no inbox should exceed the 20/day cold ceiling.
    band('maildeck_cap_integrity', 'send', num(`SELECT COUNT(*) FROM mailbox_daily_usage WHERE day=CURRENT_DATE AND sent > 20`), 1, 1, v => v === 0 ? 'all inboxes within the 20/day cold ceiling' : `${v} inbox(es) exceeded the per-day cold ceiling`);
  }
