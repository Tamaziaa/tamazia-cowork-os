...
(async () => {
  ...
  // Reoon paid-fallback budget for THIS run: cap the number of Reoon calls (only spent on free-unknowns).
  // 0 disables Reoon for the run; with no REOON_KEY each call returns 'skipped' anyway (so still £0).
  const reoonCap = Math.max(0, Number(process.env.ENRICH_VERIFY_CAP || 0));
  ...
  for (const r of rows) {
    ...
    // settled (valid/risky/invalid) is kept as-is — never re-checked, never charged. Respects ENRICH_VERIFY_CAP.
    ...
    if ((verdict === 'unknown' || !verdict) && reoonUsed < reoonCap) {
      ...
      if (rv && rv.status !== 'skipped' && rv.status !== 'error') {
        ...
        // rv.status === 'unknown' (refunded) -> leave the free 'unknown' verdict in place.
      }
    }
    ...
    pg(`UPDATE leads SET verify_status=${esc(verdict)}, deliverability=${esc(verdict)}, contact_confidence=${conf}, updated_at=NOW() WHERE id=${r.id}`);
    console.log(`  ${r.email.padEnd(40)} ${String(verdict).padEnd(8)} conf=${conf} via=${src}`);
  }
  const costNote = reoonUpgraded > 0 ? `· Reoon settled ${reoonUpgraded}/${reoonUsed} (paid fallback)` : '· £0 (no paid credits used)';
