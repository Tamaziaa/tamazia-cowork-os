...
(async () => {
  ...
  for (let _i = 0; _i < leads.length; _i += CONCURRENCY) {
    ...
    // L13 clobber guard: never re-score a human/auto-reviewed Tier-1 (apply-review stamps reviewed_at on promotion)
    if (lead.reviewed_at && lead.icp_tier === 1) { console.log(`  ${String(lead.domain || '').padEnd(30)} reviewed T1 (reviewed_at set) -> SKIP re-tier`); skipped++; continue; }
